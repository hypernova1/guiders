...
public class LoginSuccessHandler2 implements AuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		System.out.println("로그인 성공 이후 경로 결정중...........");
		clearAuthenticationAttributes(request);
		int intRedirectStrategy = decideRedirectStrategy(request, response);
		System.out.println("결정 결과........: " + intRedirectStrategy);
	...
	private void useSessionUrl(HttpServletRequest request, HttpServletResponse response) throws IOException {
		SavedRequest savedRequest = requestCache.getRequest(request, response); 
		String targetUrl = savedRequest.getRedirectUrl();  //로그인 화면 이전에 요청한 URL
